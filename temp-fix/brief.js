...
async function generateAndStore(now) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`AI service error: ${err?.error?.message || res.status}`)
  }
...
module.exports = async function handler(req, res) {
  ...
  // ── GET ?action=tweet — 6 tweet angles for a high-conviction result.
  // Admin-only (same ADMIN_CLERK_IDS pattern as the POST branch below, not
  // a hardcoded ID — the original TweetShare.jsx had the admin Clerk ID
  // literal in frontend code, which both duplicates the real source of
  // truth and ships it to every visitor's browser bundle).
  // FIX: this branch MUST come before the generic ?ticker= branch below —
  // a tweet request also includes ticker= (needed for the tweet content),
  // so it was being caught by the earlier, more general check first and
  // never reaching this one at all. Confirmed live: the endpoint was
  // silently returning {sr, brief} instead of {tweets}, which is why
  // Share never showed anything once clicked. ──
  if (req.method === 'GET' && req.query.action === 'tweet') {
    ...
    const strikeStr = req.query.strikeStr || ''
    const score     = req.query.score || '90'
    const grade     = req.query.grade || 'A'
    const dte       = req.query.dte || ''
    const target    = req.query.target || ''
    const iv        = req.query.iv || ''
    if (!ticker) return res.status(400).json({ error: 'Missing ticker' })

    try {
      const tweets = await getTweetAngles({ ticker, tradeType, strikeStr, score, grade, dte, target, iv })
      return res.status(200).json({ tweets })
    } catch (e) {
      console.error('[brief] tweet generation error:', e.message)
      return res.status(500).json({ error: e.message })
    }
  }

  // ── GET ?ticker=NVDA — S&R levels + AI ticker brief for manual scan ────────
  // Note: req.query.action !== 'tweet' guard added as a second layer of
  // defense — even if branch order ever changes again, this branch should
  // never claim a request that's explicitly asking for the tweet action.
  if (req.method === 'GET' && req.query.ticker && req.query.action !== 'tweet') {
    const { clerkId } = await getAuth(req)
    ...
    const tradeType = req.query.tradeType || 'Call'
    // Auto-scanner hit expansion only needs S/R levels (no UI space for the
    // AI brief there) — skipBrief=1 avoids a wasted Claude Haiku call when
    // the result would be discarded client-side anyway. Manual scan never
    // sets this, so its behavior (sr + brief) is unchanged.
    const skipBrief = req.query.skipBrief === '1'

    if (!ticker) return res.status(400).json({ error: 'Missing ticker' })

    try {
      const sr = await getSRLevels(ticker)
      const brief = skipBrief ? null : await getTickerBrief({
