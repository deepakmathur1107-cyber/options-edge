...
export default function MorningBrief({ getToken, theme, isAdmin, onBriefLoaded }) {
  ...
  const biasBg     = dark ? (BIAS_BG_DK[bias] || '#f59e0b14') : (BIAS_BG_LT[bias] || '#fef9c3') // currently unused — was the old bias-banner background, kept in case a future redesign wants it
  ...
  return (
    <div style={{background:cardBg,border:`1px solid ${border}`,borderRadius:10,overflow:'hidden',fontFamily:'Inter,sans-serif'}}>
      ...
      {/* Bias banner + risk trigger now live in the Dashboard hero (same
          brief.why/bias/risk_trigger data, via onBriefLoaded) — removed here
          to stop showing the identical sentence twice on one screen. Tone
          tags moved into the header strip below so they're not lost. */}
      <div style={{padding:'8px 18px',background:dark?`${biasColor}08`:`${biasColor}10`,borderBottom:`1px solid ${border}`,display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
        <span style={{fontSize:16,color:biasColor,lineHeight:1}}>{BIAS_ICON[bias]}</span>
        <span style={{fontSize:11,fontWeight:700,color:biasColor,letterSpacing:1.5,fontFamily:'IBM Plex Mono,monospace'}}>{bias.toUpperCase()}</span>
        {(brief.tone||'').split('/').map((t,i)=>(
          <span key={i} style={{fontSize:9.5,color:biasText,background:dark?`${biasColor}18`:`${biasColor}20`,border:`1px solid ${biasColor}40`,borderRadius:4,padding:'2px 7px',fontWeight:600,letterSpacing:.4,whiteSpace:'nowrap'}}>{t.trim()}</span>
        ))}
      </div>
