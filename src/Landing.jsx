...
// ── Mock scanner data — tabular format ───────────────────────────────────────
const MOCK_ALERTS = [
...
// ── Animated scanner mockup — tabular style ───────────────────────────────────
function ScannerMockup() {
  ...
  return (
    <div style={{ background:'#060c14', border:'1px solid #1a2e3e', borderRadius:10, overflow:'hidden', fontFamily:MONO, fontSize:11, boxShadow:'0 24px 64px rgba(0,255,136,0.08), 0 8px 24px rgba(0,0,0,0.6)' }}>
      ...
      {/* Table header */}
      <div style={{ padding:'6px 14px', display:'flex', gap:10, borderBottom:'1px solid #0d1e2a' }}>
        {['SYMBOL','CONTRACT','DTE','IV','CONVICTION','SCORE','GRADE','MID'].map((h,i) => (
          <span key={i} style={{ fontSize:9, color:'#2a4a5a', letterSpacing:1, flex: h==='CONVICTION' ? 1 : 'none', width: h==='SYMBOL'?36:h==='CONTRACT'?70:h==='DTE'?30:h==='IV'?30:h==='SCORE'?22:h==='GRADE'?22:36 }}>{h}</span>
        ))}
      </div>
      ...
      <div style={{ padding:'4px 0' }}>
        {MOCK_ALERTS.map((a, i) => (
          <div key={i} style={{ padding:'8px 14px', display:'flex', alignItems:'center', gap:10, background: i===active ? '#0a1e14' : 'transparent', borderLeft:'2px solid '+(i===active ? a.color : 'transparent'), transition:'all 0.4s', opacity: i===active ? 1 : 0.35 }}>
            ...
            <span style={{ color:'#4a7a8a', width:30 }}>{a.dte}D</span>
            <span style={{ color:'#4a7a8a', width:30 }}>{a.iv}</span>
