...
export default function StockWorkspace({ C, getToken }) {
  ...
  return <div className="si stock-workspace">
    ...
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:16,boxShadow:C.shadow,marginBottom:16}}>
      ...
      <div className="stock-track-grid" style={{marginTop:13}}>{[5,10,20,60].map(days=>{const metric=ratingStats?.horizons?.[days];const hasRatings=Number(ratingStats?.totalRatings)>0;return <Metric key={days} label={`${days}-SESSION RESULT`} value={metric?.sampleSize?`${metric.winRate}% wins · ${metric.averageReturn>=0?'+':''}${metric.averageReturn}%`:hasRatings?`Waiting for ${days} sessions`:'No ratings captured yet'} color={metric?.sampleSize?C.blue:C.dim} C={C}/>})}</div>
