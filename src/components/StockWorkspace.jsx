...
export default function StockWorkspace({ C, getToken }) {
  ...
  return <div className="stock-workspace">
    <style>{`
      ...
      @media(max-width:900px){.stock-hero,.stock-body,.stock-detail-grid{grid-template-columns:minmax(0,1fr)}.stock-body>section,.stock-candidate-rail{width:100%;min-width:0}.stock-candidate-rail form,.stock-candidate-rail>div:last-child>button{grid-template-columns:minmax(0,1fr) auto!important}.stock-candidate-rail>div:last-child>button>div:first-child{min-width:0}.stock-metrics,.stock-track-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.stock-candidate-rail>div:last-child{max-height:none!important;overflow-y:visible!important}}
    ...
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:16,boxShadow:C.shadow,marginBottom:16}}>
      ...
      <div className="stock-track-grid" style={{marginTop:13}}>{[5,10,20,60].map(days=>{const metric=ratingStats?.horizons?.[days];const hasRatings=Number(ratingStats?.totalRatings)>0,hasBuySetups=Number(ratingStats?.buySetups)>0;return <Metric key={days} label={`${days}-SESSION RESULT`} value={metric?.sampleSize?`${metric.winRate}% wins · ${metric.averageReturn>=0?'+':''}${metric.averageReturn}%`:hasBuySetups?`Waiting for ${days} sessions`:hasRatings?'No Buy setups yet':'No ratings captured yet'} color={metric?.sampleSize?C.blue:C.dim} C={C}/>})}</div>
      <div style={{fontSize:9,color:C.dim,lineHeight:1.5,marginTop:10}}>One forward entry per ticker with a 10-day re-entry cooldown · targets and stops checked against daily highs/lows · stocks have no expiry and remain tracked until an exit rule is reached · 5/10/20/60 sessions are review measurements, not expiration dates · compared with SPY · excludes fees · educational, not investment advice.</div>
    </div>
