...
export default function Paywall() {
  ...
  // Read trial_eligible from the subscription endpoint (no extra API call needed)
  useEffect(() => {
    const check = async () => {
      try {
        ...
        setTrialEligible(data.trial_eligible === true)
      } catch {
  ...
  const subheadline = hasTrial
    ? <>Hi {user?.firstName || 'there'}. Start your free trial to access Options Edge Pro.</>
    : <>Hi {user?.firstName || 'there'}. Your free trial has been used.<br/>Subscribe at $19/month — billed immediately, cancel anytime.</>
    ...
    <div style={{ background:'#090e14', minHeight:'100vh', fontFamily:F, color:'#c8d8e8', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'24px' }}>
      ...
      <div style={{ maxWidth:480, width:'100%', background:'#0d1a26', border:'1px solid #1a2e3e', borderRadius:8, padding:'36px 32px', textAlign:'center', marginTop:60 }}>
        ...
        <div style={{ marginBottom:20 }}>
          <div>
            <span style={{ fontFamily:BB, fontSize:40, color:'#c8d8e8', letterSpacing:1 }}>$19</span>
            <span style={{ fontSize:16, color:'#4a7a8a' }}>/month</span>
          </div>
          {!checking && trialBadge}
        </div>
      ...
      <div style={{ marginTop:20, fontSize:11, color:'#2a4a5a' }}>
        ...
        }} style={{ background:'none', border:'none', color:'#4a7a8a', cursor:'pointer', fontFamily:F, fontSize:11, textDecoration:'underline' }}>
          Manage subscription
        </button>
      </div>
    </div>
