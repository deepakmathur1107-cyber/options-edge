...
function scanTicker({ ticker, quote, expDates, chain, tf, fund, spxChg, ndxChg, srLevels = null, incumbentSide = null, trendContext = null }) {
  ...
  try {
    ...
    return {
      ...
      thetaRaw: Number.isFinite(Number(td.theta)) ? Number(td.theta) : null,
      gammaRaw: Number.isFinite(Number(td.gamma)) ? Number(td.gamma) : null,
      vegaRaw: Number.isFinite(Number(td.vega)) ? Number(td.vega) : null,
      ...
      breakevenRequiredPct: Number.isFinite(Number(breakevenReqPct)) ? Math.abs(Number(breakevenReqPct)) : null,
      expectedMovePct: td.iv > 0 && dte2 > 0 ? td.iv * Math.sqrt(dte2 / 365) * 100 : null,
      supportLevel: srLevels?.s1 ?? null,
      resistanceLevel: srLevels?.r1 ?? null,
      supportDistancePct: srLevels?.s1 && price > 0 ? Math.max(0, ((price - srLevels.s1) / price) * 100) : null,
      resistanceDistancePct: srLevels?.r1 && price > 0 ? Math.max(0, ((srLevels.r1 - price) / price) * 100) : null,
