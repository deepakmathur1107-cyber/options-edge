 ...
 *
 * Called by a Vercel Cron or manually to run the scanner and email users
 * whose alert_prefs match high-edge contracts found.
 *
...
const FROM_EMAIL = process.env.ALERT_FROM_EMAIL ?? "alerts@options-edge-theta.vercel.app";

// ─── Tradier helpers (duplicated from scan.js to keep functions independent) ──
...
function computeEdgeScore(option) {
  ...
  const liquidityScore = (open_interest ?? 0) > 500 && (volume ?? 0) > 100 ? 20 : (open_interest ?? 0) > 100 ? 10 : 0;
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#111;border:1px solid #222;border-radius:12px;overflow:hidden">
        <tr>
          <td style="padding:28px 32px;border-bottom:1px solid #222">
            <span style="font-size:20px;font-weight:700;letter-spacing:-0.5px">
              ⚡ Options <span style="color:#10b981">Edge</span> Alert
            </span>
          </td>
        </tr>
        <tr>
          <td style="padding:24px 32px">
            <p style="margin:0 0 20px;color:#a1a1aa;font-size:14px">
              High-edge contracts detected. Edge Score = composite liquidity + theta + delta quality signal.
            </p>
        ...
        <tr>
          <td style="padding:20px 32px;border-top:1px solid #222;text-align:center">
            <a href="https://options-edge-theta.vercel.app/dashboard"
        ...
        <tr>
          <td style="padding:16px 32px;border-top:1px solid #1a1a1a;text-align:center">
            <p style="margin:0;color:#52525b;font-size:11px">
              Options Edge · <a href="https://options-edge-theta.vercel.app/settings" style="color:#52525b">Manage alerts</a>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. Collect all unique symbols across subscribers
    const allSymbols = [...new Set(prefs.flatMap((p) => p.symbols ?? ["SPY", "QQQ"]))];

    // 3. Run scan for each symbol
    const scanResults = {};
    await Promise.all(
    ...
    // 4. For each subscriber, find matches above their threshold and send
    let sent = 0;
    await Promise.all(
      prefs.map(async (pref) => {
        ...
        const matching = userSymbols
          .flatMap((s) => scanResults[s] ?? [])
          .filter((c) => c.edgeScore >= minScore);
    ...
    return res.status(200).json({ sent, symbols: allSymbols, scannedAt: new Date().toISOString() });
  } catch (err) {
