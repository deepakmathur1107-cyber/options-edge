...
module.exports=async function handler(req,res) {
  ...
  try {
    ...
    for(const row of rows) {
      const quote=quotes.get(row.ticker),price=Number(quote?.last||quote?.close)
      if(!price||quoteDate(quote)!==today) continue
      const sessions=Math.min(60,Number(row.sessions_observed||0)+1)
      ...
      const next={...row,sessions_observed:sessions,last_observed_date:today,latest_price:price,latest_benchmark_price:spyPrice,
        max_return:row.max_return==null?stockReturn:Math.max(Number(row.max_return),stockReturn),
        max_drawdown:row.max_drawdown==null?stockReturn:Math.min(Number(row.max_drawdown),stockReturn),updated_at:new Date().toISOString()}
      ...
      if(!row.close_outcome&&row.target_price&&price>=Number(row.target_price)) {
        next.close_outcome='TARGET'; next.close_outcome_at=new Date().toISOString()
      } else if(!row.close_outcome&&row.stop_price&&price<=Number(row.stop_price)) {
        next.close_outcome='STOP'; next.close_outcome_at=new Date().toISOString()
      }
      updates.push(next)
    }
    if(updates.length) {
